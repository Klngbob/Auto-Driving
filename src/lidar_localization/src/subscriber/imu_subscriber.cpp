...
IMUSubscriber::IMUSubscriber(ros::NodeHandle& nh, std::string topic_name, size_t buff_size)
    :nh_(nh) {
        this->subscriber_ = this->nh_.subscribe(topic_name, buff_size, &IMUSubscriber::msg_callback, this);
    }
...
void IMUSubscriber::msg_callback(const sensor_msgs::ImuConstPtr& imu_msg_ptr) {
    ...
    this->new_imu_data_.push_back(imu_data);
}

void IMUSubscriber::ParseData(std::deque<IMUData>& imu_data_buff) {
    if(this->new_imu_data_.size() > 0) {
        imu_data_buff.insert(imu_data_buff.end(), this->new_imu_data_.begin(), this->new_imu_data_.end());
        this->new_imu_data_.clear();
    }
}
}