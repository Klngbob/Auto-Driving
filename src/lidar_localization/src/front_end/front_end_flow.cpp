...
FrontEndFlow::FrontEndFlow(ros::NodeHandle& nh) {
    ...
    this->global_pub_ptr_ = std::make_shared<CloudPublisher>(nh, "global_map", 100, "map");
...
bool FrontEndFlow::ReadData() {
    ...
    this->imu_sub_ptr_->ParseData(this->imu_data_buff_);
    this->gnss_sub_ptr_->ParseData(this->gnss_data_buff_);
    return true;
}
...
bool FrontEndFlow::PublishGlobalMap() {
    if(this->front_end_ptr_->GetNewGlobalMap(this->global_map_ptr_)) {
        this->global_pub_ptr_->Publish(this->global_map_ptr_);
        this->global_map_ptr_.reset(new CloudData::CLOUD());
    }
