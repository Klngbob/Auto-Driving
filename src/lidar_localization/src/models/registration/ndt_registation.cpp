...
NDTRegistration::NDTRegistration(const YAML::Node& node)
    :ndt_ptr_(new pcl::NormalDistributionsTransform<CloudData::POINT, CloudData::POINT>()) {
        ...
        this->SetRegistrationParam(res, step_size, trans_eps, max_iter);
}
...
bool NDTRegistration::SetRegistrationParam(float res, float step_size, float trans_eps, int max_iter) {
    this->ndt_ptr_->setResolution(res);
    this->ndt_ptr_->setStepSize(step_size);
    this->ndt_ptr_->setTransformationEpsilon(trans_eps);
    this->ndt_ptr_->setMaximumIterations(max_iter);
...
bool NDTRegistration::SetInputTarget(const CloudData::CLOUD_PTR& input_target) {
    this->ndt_ptr_->setInputTarget(input_target);
...
bool NDTRegistration::ScanMatch(const CloudData::CLOUD_PTR& input_source,
    ...
    this->ndt_ptr_->setInputSource(input_source);
    this->ndt_ptr_->align(*result_cloud_ptr, predict_pose);
    result_pose = this->ndt_ptr_->getFinalTransformation();
